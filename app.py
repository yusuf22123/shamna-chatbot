...
# دالة للبحث عن الأصناف والكميات والتفاصيل
def find_items_in_text(text):
    ...
    for i in range(len(words)):
        if words[i].isdigit():
            ...
            best_score = 0
            for item in menu:
                score = fuzz.partial_ratio(normalize_text(item), remaining_text)
                if score > 80 and score > best_score:  # 80% تشابه على الأقل
                    found_item = item
                    best_score = score
            if found_item:
    ...
    if not items:
        for item in menu:
            score = fuzz.partial_ratio(normalize_text(item), normalized_text)
            if score > 80:  # 80% تشابه على الأقل
                items.append(item)
                quantities[item] = 1
                if "بدون" in normalized_text or "من غير" in normalized_text:
                    extra = normalized_text.split("بدون")[-1] or normalized_text.split("من غير")[-1]
...
def chat():
    ...
    if state == "waiting_for_quantities":
        items, quantities, details = find_items_in_text(user_message)
        if not quantities:
            # لو مفيش كميات في النص، افترض 1 لكل صنف
            quantities = {item: 1 for item in selected_items}
        else:
            # لو فيه كميات، استخدمها واملأ الباقي بـ 1
            for item in selected_items:
